...
/**
 * Reconciles the monotonic clock (used by sensor events and CameraX capture
 * timestamps) with the wall clock (what a human/court reads), per research/03 §4.
 *
 * Why this exists: `SensorEvent.timestamp` and `ImageProxy.imageInfo.timestamp`
 * share the `elapsedRealtimeNanos` base, so they are directly comparable to each
 * other — but NOT to `System.currentTimeMillis()`. Taking one paired reading of
 * both clocks at bundle time yields an offset that converts any monotonic
 * timestamp into wall-clock time.
 *
